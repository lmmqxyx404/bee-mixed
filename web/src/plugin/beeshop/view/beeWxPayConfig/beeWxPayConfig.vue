...
<!--        <el-table-column align="left" label="v3证书私钥" prop="privateCert" width="120" />-->
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeWxPayConfigFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="调试模式:"  prop="debug" >
              <el-switch v-model="formData.debug" active-color="#13ce66" inactive-color="#ff4949" active-text="是" inactive-text="否" clearable ></el-switch>
            </el-form-item>
...
import {
  ...
  deleteBeeWxPayConfig,
  deleteBeeWxPayConfigByIds,
  updateBeeWxPayConfig,
  findBeeWxPayConfig,
  getBeeWxPayConfigList
...
const getTableData = async() => {
  const table = await getBeeWxPayConfigList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeWxPayConfigByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeWxPayConfigFunc = async(row) => {
    const res = await findBeeWxPayConfig({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeWxPayConfigFunc = async (row) => {
    const res = await deleteBeeWxPayConfig({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
